...
def transform_df_and_export_parquet(
    ...
    # Calculate highest change in consumption between invoices   
    df_max_consomm_cols = daf.change_in_consommation(df_invoice)
    # Merge max change with client
    df_client_w_max_consomm = pd.merge(
        df_client, df_max_consomm_cols, how="left", on="client_id"
    )
   
    # Combine aggregated invoice data with client data
    df_client_merged_w_agg_invoice = pd.merge(
        df_client_w_max_consomm, df_invoice_aggregated, how="left", on="client_id"
    )
...
transform_df_and_export_parquet(
    client_train,
    invoice_train,
    "data/df_train_no_dummies_agg_max_consomm.parquet",
    "data/df_train_no_dummies_non_agg.parquet",#not used - commented out above
)
transform_df_and_export_parquet(
    client_final_test,
    invoice_final_test,
    "data/df_final_test_no_dummies_agg_max_consomm.parquet",
    "data/df_final_test_no_dummies_non_agg.parquet",#not used - commented out above
)
